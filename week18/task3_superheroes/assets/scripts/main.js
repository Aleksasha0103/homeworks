const superheroesArray = [
  ...
  },
  {
    ...
    Gfriends: "частично Мстители, Человек-паук, Росомаха",
...
const allSuperherosDiv = document.getElementById("allSuperheros");

document.addEventListener("DOMContentLoaded", () => {
  superheroesArray.forEach((hero) => {
    const superheroDiv = document.createElement("div");
    superheroDiv.classList.add("superhero");
    allSuperherosDiv.append(superheroDiv);

    const workingDiv = document.createElement("div");
    workingDiv.classList.add("superhero__working-div");
    superheroDiv.append(workingDiv);
    ...
    workingDiv.append(superheroName);

    const buttonMoreInfo = document.createElement("button");
    buttonMoreInfo.classList.add("superhero__button-more-info");
    buttonMoreInfo.textContent = "Click!";
    workingDiv.append(buttonMoreInfo);

    const superheroUniverse = document.createElement("p");
    superheroUniverse.classList.add("superhero__info");
    superheroUniverse.textContent = `Вселенная: ${hero.universe}`;
    superheroDiv.append(superheroUniverse);

    const superheroAlterego = document.createElement("p");
    superheroAlterego.classList.add("superhero__info");
    superheroAlterego.textContent = `Альтер эго: ${hero.alterego}`;
    superheroDiv.append(superheroAlterego);

    const superheroOccupation = document.createElement("p");
    superheroOccupation.classList.add("superhero__info");
    superheroOccupation.textContent = `Род деятельности: ${hero.occupation}`;
    superheroDiv.append(superheroOccupation);

    const superheroFriends = document.createElement("p");
    superheroFriends.classList.add("superhero__info");
    superheroFriends.textContent = `Друзья: ${hero.friends}`;
    superheroDiv.append(superheroFriends);

    const superheroSuperpowers = document.createElement("p");
    superheroSuperpowers.classList.add("superhero__info");
    superheroSuperpowers.textContent = `Суперсила: ${hero.superpowers}`;
    superheroDiv.append(superheroSuperpowers);

    const superheroImage = document.createElement("img");
    superheroImage.classList.add("superhero__img");
    superheroImage.src = hero.url;
    superheroImage.alt = hero.name;
    superheroDiv.append(superheroImage);

    const starRatingDiv = document.createElement("div");
    starRatingDiv.classList.add("superhero__star-rating-div");
    superheroDiv.append(starRatingDiv);

    for (let i = 1; i <= 5; i++) {
      const starCheckboxLabel = document.createElement("label");
      starCheckboxLabel.htmlFor = `${hero.name}Star${i}`;
      starRatingDiv.append(starCheckboxLabel);

      const starCheckboxInput = document.createElement("input");
      starCheckboxInput.classList.add("superhero__star-input");
      starCheckboxInput.type = "checkbox";
      starCheckboxInput.id = `${hero.name}Star${i}`;
      starCheckboxLabel.append(starCheckboxInput);

      const starCustomCheckbox = document.createElement("span");
      starCustomCheckbox.classList.add("star-custom-checkbox");
      starCheckboxLabel.append(starCustomCheckbox);
    }
    buttonMoreInfo.addEventListener("click", () => openMoreInfo(hero));
  });

  function openMoreInfo(hero) {
    const popUpMoreInfo = document.createElement("div");
    popUpMoreInfo.classList.add("superhero__pop-up-more-info");
    const popUpMoreInfoText = document.createElement("p");
    popUpMoreInfoText.classList.add("superhero__pop-up-more-info__text");
    popUpMoreInfoText.textContent = `Подробная информация: ${hero.info}`;
    document.superheroDiv.append(popUpMoreInfo);
    popUpMoreInfo.append(popUpMoreInfoText);
  }
});
